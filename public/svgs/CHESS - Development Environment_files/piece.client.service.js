	...
	function() {
		return function Piece () {
			...
			this.getMoves = function(board, strict) {
				...
				for (i; i < 64; i += 1) {
					if (strict === 1) {
						if (this.valid(i, board) && this.clearPath(this.path(i), board) && !this.wouldBeInCheck(board, i)) {
							squares.push(i);
						} 
					} else if (strict === 2) {
						if (this.valid(i, board) && this.clearPath(this.path(i), board)) {
							squares.push(i);
						}
					} else if (strict === 0) {
